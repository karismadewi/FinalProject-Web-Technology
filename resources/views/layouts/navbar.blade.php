...
<nav class="px-2 sm:px-4 py-2.5 bg-white border-b border-slate-200">
    <div class="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default " method="POST" action="{{ route('logout') }}">
            @csrf
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-dropdown-link :href="route('logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();" class="block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-teal-600 md:p-0">
                        {{ __('Log Out') }}
                    </x-dropdown-link>
                </form>
            </ul>
